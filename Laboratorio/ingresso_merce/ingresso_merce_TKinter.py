...
def _ctk_image(path, size):
    """Carica un'immagine per i widget CustomTkinter."""
    pil = Image.open(path)
...
def setup_window(window):
    """Configura la finestra principale per il modulo Ingresso Merce."""
...
def build_ui(app):
    """Costruisce la UI per IngressoMerce.

    La funzione presuppone che l'istanza `app` abbia già inizializzato dati e
    variabili (ad esempio liste, variabili Tk e callback dei bottoni).
    """
    ...
    tree_wrap = ctk.CTkFrame(app.labelframe_elenco, fg_color="transparent", corner_radius=0)
    ...
    tabs = app.notebook_dettagli.tabs()
    tab_dati_id = str(app.tab_dettagli_dati)
    if tab_dati_id in tabs:
    ...
    for col in (
        ...
        app.tree_elenco.heading(col, text="")
    ...
    for col in ("prog_acq", "num_ddt", "taglio", "peso_i", "peso_f", "lotto_chiuso", "id_merc"):
        app.tree_elenco.column(col, width=0, minwidth=0, stretch=False, anchor="center")
