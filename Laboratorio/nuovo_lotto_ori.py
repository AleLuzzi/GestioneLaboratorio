...
class NuovoLotto(tk.Toplevel):
    def __init__(self):
        ...
        self.f = open('../laboratorio/prog_lotto_ven.txt', "r")
        self.prog_lotto_ven = self.f.read()
        self.f.close()
        ...
        for row in self.c.execute("SELECT prodotto FROM prodotti WHERE reparto = 3"):
    ...
    def esci_salva(self):
        if (self.nuova_produzione.get() != '') \
            ...
            self.conn.commit()
            self.conn.close()
            file = open('../laboratorio/prog_lotto_ven.txt', "w")
            file.write(str(int(self.prog_lotto_ven) + 1))
            file.close()
            self.destroy()
        else:
            pass
